...
(function ($, window, i) {
    ...
    $.fn.editTable = function (options) {
        ...
        function buildCell(content, type) {
            ...
            // Default
            return '<td><input type="text" id="cid'+String(cid++)+'" value="' + content.toString().replace(/"/g, "&quot;") + '" /></td>';
        }
        ...
        // Add column
        $table.on('click touchend', '.addcol', function () {
        ...
        // Remove column
        $table.on('click touchend', '.delcol', function () {
        ...
        // Add row
        $table.on('click touchend', '.addrow', function () {
            ...
            checkButtons();
            for(i=0;i<cid;++i)
            $("#cid"+String(i)).autocomplete({ 
                source: OnscreenList });
            return false;
        });

        // Delete row
        $table.on('click touchend', '.delrow', function () {
        ...
        $table.on('click touchend', 'input', function () {
            $(this).select();
        });
